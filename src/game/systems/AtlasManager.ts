...
export class AtlasManager {
  ...
  public createEffectAnimations(): void {
      ...
      // Gnoll Claw
      if (this.scene.textures.exists('gnoll-claw-1')) {
          const clawFrames = [];
          for (let i = 1; i <= 12; i++) {
              if (this.scene.textures.exists(`gnoll-claw-${i}`)) {
  ...
  private createKeyFrameAnimation(key: string, count: number, prefix: string, rate: number, repeat: number, pad3 = false): void {
      if (this.scene.anims.exists(key)) return;
      
      const frames = Array.from({ length: count }, (_, i) => {
          const numStr = pad3 ? String(i).padStart(3, '0') : String(i);
          return { key: `${prefix}${numStr}` };
      });
